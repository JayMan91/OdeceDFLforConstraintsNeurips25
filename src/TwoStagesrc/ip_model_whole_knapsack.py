...
from src.TwoStagesrc.remove_redundancy import _remove_redundancy, _remove_redundancy_sparse, _remove_redundancy_dense
...
def _initialization(shape, init_val=None):
    if init_val is None:
        ...
        tau0 = 1. #np.array([1], dtype=np.float64)
        kappa0 = 1. #np.array([1], dtype=np.float64)
    else:
        x0 = init_val['x']
        y0 = init_val['y']
        t0 = init_val['t']
...
def IPOfuncKnapsack(A =None,b =None,h=None,c=None,GTrue=None,purchase_fee=0,compensation_fee=0,alpha0=0.9995,beta=0.1,pc = True,
    tol= 1e-8,max_iter=1000,init_val= None,method =1,mu0=None, dim_kp = 1,
    ...
    itemNum = GTrue.shape[1]
    varNum = itemNum

    class IPOfunc_cls(Function):        
        @staticmethod
        def forward(ctx,GTemp):
#            print(cGTemp.shape)  # torch.Size([10, 2])
            
            G = torch.zeros((itemNum + dim_kp, itemNum))
            # c = - purchase_fee * cGTemp[:, 0]
            for i in range(itemNum):
                G[i][i] = 1
            for i in range(dim_kp):
                G[itemNum + i] = GTemp[i, :]
...
#                print(c_list, G_list, h_numpy, x_sol)
                ...
                if m2.Status == GRB.OPTIMAL:
                    ...
                    ym2_val = [ y_m2[i].x for i in range(varNum)]
                else:
                    raise Exception("No soluton found")
                    # print ("x_sol", x_sol)
                    ym2_val =  np.zeros(varNum , dtype=np.float64)
...
#            print(trueA.shape) # torch.Size([0, 48]) torch.Size([0, 242])
            ...
            G_numpy = np.zeros((dim_kp, itemNum))
            for i in range(dim_kp):
                G_numpy[i] = G_numpyTemp[itemNum + i]
            h_numpy = h_numpyTemp[ itemNum: ]
            rowSizeG = G_numpy.shape[0]
#            print(n_var)
...
#             print ("SHAPE", G_numpy.shape, hGx2.shape)
#             H2 = G_numpy.T.dot(hGx2).dot(G_numpy)
#             H2 = mu * H2
...
#            print(cTrue)
            ...
            for i in range(colSizeG):
                dRxs2[i] = purchase_fee * c[i] + (compensation_fee - purchase_fee) * c[i]
                dRxs1[i] = -(compensation_fee - purchase_fee) * c[i]
#                if x_sol[i] > x_s1[i]:
...
#            print('Compute G_grad cost:',time_end5-time_end4,'s')
            # changingbecause in our datset, we have to return gradient of G only
            cG_grad = torch.zeros(( dim_kp, n_var))
            # cG_grad[:, 0] = c_grad
            # cG_grad[:, 1] = G_grad
            cG_grad[:] = G_grad
#            if any(torch.isnan(c_grad).tolist()):
...
#                logging.info("Inf in bkwd pass ; del_x contains Inf?- {}".format(any(torch.isinf(del_x).tolist())))
    ...
    IPOfuncKnapsack.Runtime = Runtime
    IPOfuncKnapsack.end_vectors = end_vectors
    IPOfuncKnapsack.forward_solved = forward_solved    
